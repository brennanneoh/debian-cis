...
audit() {
    ...
    local nonroot_gid0 filtered_users found_exceptions l_user
    nonroot_gid0=$(awk -F: '($1 !~ /^(root|sync|shutdown|halt|operator)$/ && $4=="0") {print $1}' /etc/passwd || true)
    filtered_users=""
    found_exceptions=""

    for l_user in $nonroot_gid0; do
        if echo "$EXCEPTIONS" | grep -qw "$l_user"; then
            found_exceptions="$found_exceptions $l_user"
        else
            filtered_users="$filtered_users $l_user"
        fi
    done

    ROG0_NONROOT_USERS=$(xargs <<<"$filtered_users")

    if [ -z "$ROG0_NONROOT_USERS" ]; then
        ok "No non-root account has primary GID 0${found_exceptions:+ apart from configured exceptions:}$found_exceptions"
        ROG0_ONLY_ROOT_OK=0
    else
...
create_config() {
    cat <<EOF
status=audit
# Put here valid accounts with primary GID 0 separated by spaces
EXCEPTIONS=""
EOF
}
