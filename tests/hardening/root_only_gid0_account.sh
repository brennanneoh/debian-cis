...
test_audit() {
    ...
    sed -i '/^EXCEPTIONS=/d' "${CIS_CONF_DIR}/conf.d/${script}.cfg"
    echo 'EXCEPTIONS="testuser_gid0"' >>"${CIS_CONF_DIR}/conf.d/${script}.cfg"
    register_test retvalshouldbe 0
    register_test contain "configured exceptions"
    run exception_configured "${CIS_CHECKS_DIR}/${script}.sh" --audit-all

    sed -i '/^EXCEPTIONS=/d' "${CIS_CONF_DIR}/conf.d/${script}.cfg"
    echo 'EXCEPTIONS=""' >>"${CIS_CONF_DIR}/conf.d/${script}.cfg"
